...
class Skill(Base):
    ...
    topics_json: Mapped[list[str] | None] = mapped_column(JSON)
    skill_type: Mapped[str | None] = mapped_column(String(64), index=True)
    platforms: Mapped[list[str] | None] = mapped_column(JSON)
    capabilities: Mapped[list[str] | None] = mapped_column(JSON)
    install_methods: Mapped[list[str] | None] = mapped_column(JSON)
    config_keys: Mapped[list[str] | None] = mapped_column(JSON)
    source_files: Mapped[list[str] | None] = mapped_column(JSON)
    readme_excerpt: Mapped[str | None] = mapped_column(Text)
    quality_score: Mapped[int | None] = mapped_column(Integer, index=True)
    verification_status: Mapped[str | None] = mapped_column(String(64), index=True)
